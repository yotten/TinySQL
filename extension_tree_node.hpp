...
public:
	struct ExtensionTreeNode *parent = nullptr; //!< 親となるノードです。根の式木の場合はnullptrとなります。
	struct ExtensionTreeNode *left = nullptr;   //!< 左の子となるノードです。自身が末端の葉となる式木の場合はnullptrとなります。
	Operator middleOperator;             		//!< 中置される演算子です。自身が末端のとなる式木の場合の種類はNOT_TOKENとなります。
	struct ExtensionTreeNode *right = nullptr;  //!< 右の子となるノードです。自身が末端の葉となる式木の場合はnullptrとなります。
